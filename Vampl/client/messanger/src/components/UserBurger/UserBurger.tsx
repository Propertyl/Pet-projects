...
import { BurgerInfo,BurgerText,DefaultRef,Store } from "../types/global";
            ...
            <div className="container flex-center" style={{flexDirection:'column'}}>
              ...
              <div className="head-main-info">
                ...
                <p className="user-burger-status flex-center">
                  {burgerInfo?.status?.text ?? 'Recently'}
                  <span className={`user-burger-status-dot status-${burgerInfo?.status?.class}`}></span>
                </p>
              </div>
            </div>
          </div>
...
const EditMode = ({mode,switchMode}:{mode:boolean | null,switchMode:Dispatch<SetStateAction<boolean | null>>}) => {
  const {burgerInfo,setBurgerInfo,userBirthDate,setUserBirthDate,pageText} = useBurgerContext();
  ...
  const offMenu = () => {
    ...
    if(tempFile.current || newName || userBirthDate) {
      setBurgerInfo(null);
  ...
  const setupAvatar = (event:React.ChangeEvent<HTMLInputElement>) => {
  ...
  useDebounceEffect(() => {
    if (newName && burgerInfo?.name) {
      checkName(newName,setDataCorrect,dispatch,burgerInfo.name);
    }
  },[newName],500);
...
const BurgerBody = () => {
  ...
  const {burgerInfo,setBurgerInfo,setPageText,pageText} = useBurgerContext();
                ...
                setBurgerInfo((currentInfo:null | BurgerInfo) => {
                  if(currentInfo) {
                    return {...currentInfo,status:{
                      text:convertStatus(status,pageText?.user_status.online,pageText?.user_status.offline),
                      class:convertStatus(status,'Online','Offline')
                    }}
                  }
