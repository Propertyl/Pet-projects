...
const EditMode = ({mode,switchMode}:{mode:boolean | null,switchMode:Dispatch<SetStateAction<boolean | null>>}) => {
  const {burgerInfo,setBurgerInfo,userBirthDate,setUserBirthDate} = useBurgerContext();
  ...
  useEffect(() => {
    if(burgerInfo !== null && burgerInfo?.status === undefined) {
      const getStatus = async () => {
        if(burgerInfo) {
          await queryRequest(dataApi,'getBurgerData',{url:'status',param:burgerInfo.phone},dispatch)
