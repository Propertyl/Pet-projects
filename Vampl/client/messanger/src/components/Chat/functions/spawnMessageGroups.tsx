...
import { UserData, SetDispatch, DateGroup, ObserverProps, Message } from "../../types/global"
...
export class ChatProcess {
  ...
  setGroups:SetDispatch<Record<string,ReactElement>[]>
  setContextMenu:SetDispatch<boolean>
  setContextMenuPos:SetDispatch<{x:number,y:number}>
  ...
  spawnGroup = (
    ...
    return (
      <div className="container container-reverse group-container" key={`group-${index}`}>
               ...
               {Object.values(date).pop()!.groups.map((groupData:DateGroup,groupIdx:number,arr:DateGroup[]) => {
 ...
 spawnGroups() {
    ...
    return () => {
      ...
      for(const [index,date] of currentSlice) {
        this.setGroups((elemGroups:Record<string,ReactElement>[]) => {
          const [dateKey] = Object.keys(date);
          const newGroups = [...elemGroups];
          const group:Record<string,ReactElement> = {[dateKey]:this.spawnGroup(Number(index),date)};
