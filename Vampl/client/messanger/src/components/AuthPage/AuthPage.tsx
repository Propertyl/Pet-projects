...
const restoreCursorPosition = (el:any, cursorPosition:any) => {
  ...
  const signActions = async () => {
     switch(currentWindow) {
        case 'phone':
         ...
         const res:any = await queryRequest(userApi,'getUserConvenientData',{url:'phone',param:currentPhone},dispatch); 
        //  serv.get(`/user/phone/${currentPhone}`);
        ...
        setInput('');
        const authReq:any = await auth(register ?? false,finalData,dispatch);
        if(!authReq) {
  ...
  useEffect(() => {
    const setupPage = async () => {
       const fetchedCodes:any = await queryRequest(dataApi,'getBurgerData',{url:'phoneCodes'},dispatch); 
      //  serv.get("/getData/phoneCodes");
       setPhoneCodes((_:any) => {
        const elements = [];
                  ...
                  <div className="input-container">
                    ...
                    { inputError === 'password' && 
                      <div className="error-dialog">
                        ...
                        <p className="auth-label  flex-center">A-Z letter</p>
                        <p className="auth-label  flex-center">0-9 number</p>
                        <p className="auth-label  flex-center">minimum 8 symbols</p>
                      </div>
                    }
                  </div>
                </li>
