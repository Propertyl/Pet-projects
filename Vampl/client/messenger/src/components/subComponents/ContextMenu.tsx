...
import { defaultCoords, DefaultRef, SetDispatch } from "../types/global";
import touchOutOfMenu from "../global-functions/touchOutOfMenu";
import unScrollDocument from "../global-functions/unScrollDocument";

const makeItInBounds = (extremePoint:number,windowBound:number,menuCoord:number) => {
...
const controlOutOfBoundsSpawn = (pos:defaultCoords,menu:DefaultRef):defaultCoords => {
  ...
  newPos.x = makeItInBounds(rect.right,window.innerWidth,newPos.x);
  newPos.y = makeItInBounds(rect.bottom,window.innerHeight,newPos.y);

  return newPos;
}

const ContextMenu = ({func,pos,switchState,phrase}:{func:() => () => void,pos:{x:number,y:number},switchState:SetDispatch<boolean>,phrase:string}) => {
